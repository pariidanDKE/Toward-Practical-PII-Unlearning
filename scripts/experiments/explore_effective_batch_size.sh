...
### This script runs PerMU across different gradient accumulation steps to evaluate sensitivity to effective batch size.
...
export project_name="GradAccumCompare"
...
# Loop through different gradient accumulation steps
for gradaccum in 10 12
do
    ...
    export save_dir="$PWD/experiment/${dataset}/${model}/${split}/${run_name}_answer_tagging"

    echo "Running model with batch_size=${batch_size} and grad_accum=${gradaccum} (effective_batch_size=${effective_batch_size})"
    ...
    echo "Finished run for batch_size=${batch_size}, grad_accum=${gradaccum} (effective_batch_size=${effective_batch_size})"
    echo "--------------------------------------------------------"
done

echo "Finished all gradient accumulation sweeps"
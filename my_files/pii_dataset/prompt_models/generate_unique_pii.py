...
import pandas as pd
from prompt_gen_utils import generate_pii_prompt,get_generated_pii_json_schema,system_prompt_generate_pii
...
qapair_json_schema = get_generated_pii_json_schema()
...
raw_prompts = generate_pii_prompt(add_noise=False)
prompts = []

for prompt in raw_prompts:
    chat = [
    {"role": "system", "content": system_prompt_generate_pii},
...
results_df = pd.DataFrame(columns=['prompt','region','pii_category','generated_text'])

# Collect results
for i, output in enumerate(outputs):
    prompt = output.prompt
    generated_text = output.outputs[0].text
    results_df.loc[i] = [prompts[i],raw_prompts[i]['region'],raw_prompts[i]['pii_category'],generated_text]
    print(f"Prompt {i+1}/{len(outputs)} processed")

# Save results to CSV
file_name = f'UniquePII'

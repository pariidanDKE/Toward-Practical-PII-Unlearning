...
class PhiModel(PhiPreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Initialize layer counter for noise injection
        tot = 0
...
class PhiForCausalLM(PhiPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        Args:
            tokens_to_mix (`List`, *optional*):
                List of token ranges to apply noise injection to. Each element should contain tuples of (start, end, _).
            layer (`List`, *optional*):
                List of layer indices where noise injection should be applied.
            noise (`float`, *optional*):
                Noise scaling factor or noise function for controlled noise injection.
        ...
        # Enhanced logic for output_hidden_states with multi-layer logits and losses
        if output_hidden_states:
            all_layer_logits = []
            new_all_hidden_states = outputs.hidden_states
            for hid in new_all_hidden_states:
                # Apply the same slicing logic to each hidden state
                log = self.lm_head(hid[:, slice_indices, :])
                all_layer_logits.append(log)

            all_loss = []
            if labels is not None:
                for layer_logits in all_layer_logits:
                    # Compute loss for each layer's logits

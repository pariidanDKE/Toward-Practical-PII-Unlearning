    ...
    def generate(
        ...
        **kwargs,
        ...
        if not do_sample:
            #! Greedy
            self.logits_processor = LogitsProcessorList([
                MinLengthLogitsProcessor(15, eos_token_id=self.tokenizer.eos_token_id),  # basellm.generation_config
            ])
...
class ULDLLM(torch.nn.Module, ContrastGenerationMixin):
    def __init__(self, basellm : AutoModelForCausalLM, assist_llm : AutoModelForCausalLM, weight : float, top_logit_filter=0.0, tokenizer=None) -> None:
        super().__init__()
        self.basellm = basellm
        self.assist_llm = assist_llm
        self.weight = weight

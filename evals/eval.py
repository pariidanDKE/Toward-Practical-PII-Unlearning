    ...
    for attempt in range(3):
        ...
        # do thing
            if cfg.use_pretrained:
                model = AutoModelForCausalLM.from_pretrained(model_id, config=config, use_flash_attention_2=model_cfg["flash_attention2"]=="true", \
                    torch_dtype=torch.bfloat16, trust_remote_code = True, device_map=device_map)
            else:
